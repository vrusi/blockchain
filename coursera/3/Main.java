...
public class Main {
    ...
    private static UTXOPool getUTXOPoolFromTx(Transaction tx) {
        if (tx == null)
            return null;
    ...
    public static void main(String[] args) throws NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        ...
        System.out.println("genesis value " + genesis.getTransactions().stream().mapToDouble(tx -> tx.getOutputs().stream().mapToDouble(o -> o.value).sum()).sum());


        tx1.addOutput(5, pk_alice.getPublic());
        tx1.addOutput(10, pk_alice.getPublic());
        tx1.addOutput(10, pk_alice.getPublic());
        System.out.println("Block1 value " + block1.getTransactions().stream().mapToDouble(tx -> tx.getOutputs().stream().mapToDouble(o -> o.value).sum()).sum());
        ...
        UTXOPool utxoPool = bc.getMaxHeightUTXOPool();
